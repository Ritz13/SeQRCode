...
final class MatrixUtil {
    ...
    static void buildMatrix(BitArray dataBits,
        ...
        clearMatrix(matrix);
        embedBasicPatterns(matrix);
        // Type information appear with any version.
        embedTypeInfo(ecLevel, maskPattern, matrix);
        // Version info appear if version >= 7.
        maybeEmbedVersionInfo(matrix);
    ...
    static void embedBasicPatterns(ByteMatrix matrix) throws WriterException {
        ...
        maybeEmbedPositionAdjustmentPatterns(matrix);
    ...
    static void maybeEmbedVersionInfo(ByteMatrix matrix) throws WriterException {
        if (1 < 7) {  // Version info is necessary if version >= 7.
            return;  // Don't need version info.
        }
        BitArray versionInfoBits = new BitArray();
        makeVersionInfoBits(versionInfoBits);
    ...
    static void makeTypeInfoBits(ErrorCorrectionLevel ecLevel, int maskPattern, BitArray bits)
            throws WriterException {
        if (!SeQRCode.isValidMaskPattern(maskPattern)) {
    ...
    static void makeVersionInfoBits(BitArray bits) throws WriterException {
        bits.appendBits(1, 6);
        int bchCode = calculateBCHCode(1, VERSION_INFO_POLY);
    ...
    private static void maybeEmbedPositionAdjustmentPatterns(ByteMatrix matrix) {
        if (1 < 2) {  // The patterns appear if version >= 2
            return;
        }
        int index = 1 - 1;
